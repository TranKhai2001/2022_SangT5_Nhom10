...
import com.example.the_final_term20220216.functions.SendMail;
...
public class ForgetPassServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        boolean checkExistMail = SendMail.checkExistMail(email);
...
//            request.setAttribute("err",new String("email không tồn tại"));
        ...
        String path = request.getServerName()+":"+request.getServerPort()+request.getContextPath()+"/GetPassWordWithOTP?email="+email+"&otp="+otp;
...
//        request.setAttribute("err",new String("email đã được gửi đi"));
    ...
    private int generateRandomNumber(String email){Random rand = new Random();
