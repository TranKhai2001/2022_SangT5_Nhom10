...
public class AddCart extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        boolean check = CartFunctions.checkProductCart(id_user,id_product);
        boolean result = false;
        System.out.println("[OUTSIDE]id_user: "+id_user+", idproduct: "+id_product+", amount: "+amount +", CHECK: "+check);
        if(check == true){
            int amount_cart = CartFunctions.getAmount(id_user,id_product);
            int amountCheck = amount+amount_cart;
            if(amountCheck==0){
                CartFunctions.deleteProduct(id_user,id_product);
                result = true;
            }else {
                result = CartFunctions.updateAmount(id_user, id_product, amount);
            }
        }if(check==false){
            System.out.println("id_user: "+id_user+", idproduct: "+id_product+", amount: "+amount);
            result = CartFunctions.insertProduct(id_user,id_product,amount);
        }
